...
class FlowTriggerServiceTest {
    ...
    void shouldBuildExecutionFromFlowResolvedForRuntimeWhenAFlowTriggerMatches() {
        ...
        FlowWithSource asAuthored = flowWithFlowTriggerSource();
        FlowWithSource resolved = asAuthored.toBuilder()
            .labels(List.of(new Label("team", "platform")))
            .variables(Map.of("env", "prod"))
            .build();
        when(flowMetaStore.findByIdThenInjectDefaults(MAIN_TENANT, TEST_NAMESPACE, asAuthored.getId(), Optional.of(1)))
        ...
        // When
        var resultingExecutionsToRun = flowTriggerService.computeExecutionsFromFlowTriggerConditions(simpleFlowExecution, asAuthored);

        // Then the execution snapshots the resolved flow, not the authored one
    ...
    void shouldNotResolveFlowForRuntimeWhenNoFlowTriggerMatches() {
        ...
        assertThat(resultingExecutionsToRun).isEmpty();
        verify(flowMetaStore, never()).findByIdThenInjectDefaults(any(), any(), any(), any());
    }
    ...
    void shouldStillCreateExecutionWhenResolvedFlowIsBlocked() {
        ...
        FlowWithSource asAuthored = flowWithFlowTriggerSource();
        when(flowMetaStore.findByIdThenInjectDefaults(MAIN_TENANT, TEST_NAMESPACE, asAuthored.getId(), Optional.of(1)))
            .thenReturn(Optional.of(FlowWithException.from(asAuthored, new FlowBlockedException("Blocked by governance policy"))));
        var simpleFlowExecution = Execution.newExecution(aSimpleFlow(), EMPTY_LABELS).withState(State.Type.SUCCESS);
    ...
    @Test
    void shouldPreferResolvedLabelOverAuthoredOneWhenAFlowTriggerMatches() {
        // Given a flow whose authored label governance overrides at runtime
        FlowWithSource asAuthored = flowWithFlowTriggerSource().toBuilder()
        ...
        // Then the override wins, so the authored value never pins itself back onto the execution
        assertThat(resultingExecutionsToRun).hasSize(1);
