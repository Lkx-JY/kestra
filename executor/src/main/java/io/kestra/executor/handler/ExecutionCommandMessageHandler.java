...
public class ExecutionCommandMessageHandler implements ExecutorMessageHandler<ExecutionCommand> {
    ...
    private Optional<ExecutorContext> handleCreate(Create command) {
        ...
        String error = null;
        try {
            // the flow must be the one the executor will run, not the one as authored: the new execution
            // snapshots its labels and variables, which plugin defaults and governance can still change
            var flow = flowMetaStore
                .findByIdThenInjectDefaults(command.tenantId(), command.namespace(), command.flowId(), Optional.ofNullable(command.flowRevision()))
    ...
    private Optional<ExecutorContext> handleReplay(Replay command) {
        ...
        try {
            ...
            if (command.revision() != null) {
                flow = flowMetaStore
                    .findByIdThenInjectDefaults(command.tenantId(), command.namespace(), command.flowId(), Optional.of(command.revision()))
